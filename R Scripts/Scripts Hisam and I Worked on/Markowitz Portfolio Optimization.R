#I want to implement this after I have found a way to minimize my stock universe. Potential ways include: technical analysis, sentiment analysis, computational methods, fundemental analysis. 

##Markowitz's Mordern Portfolio Theory
install.packages("stockPortfolio")
#we will use this package to download stock data
install.packages("quadprog")
#we will use this package to optimize the portfolio
#load in the packages
library(stockPortfolio)
library(quadprog)
#Downlaod the returns of stocks from Yahoo! finance:
returns<-getReturns(c("BAC","COST","WFM"), freq="week")
...
eff <- eff.frontier(returns=returns$R, short="no", max.allocation=1, risk.premium.up=.5, risk.increment=.001)
...
 geom_point(data=eff.optimal.point, aes(x=Std.Dev, y=Exp.Return, label=sharpe), color=ealred, size=1) +
...
 plot.title=element_text(size=10, color=ealred))
